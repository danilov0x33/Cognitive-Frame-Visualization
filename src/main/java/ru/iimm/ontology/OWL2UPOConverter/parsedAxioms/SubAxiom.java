...
/**
 * Субаксиома - часть составной аксиомы. Субаксиома может быть либо простой -
 * состоять из одного именованного класса или составной - коньюнкция классов и
 * других classExpressions. Субаксиома представляется в ОПП отдельным
 * экземпляром и связывается с субаксиомами (простыми и сложными), включенными в
 * нее. Субаксима является отражением OWLClassExpression из OWLapi.
 * 
 * @author Lomov P.A.
 *
 */
...
// public interface SubAxiom extends ShareInf_thsAPI
{
	...
	/**
	 * OWL выражение, из которого создается данный экземпляр субаксиомы.
	 */
