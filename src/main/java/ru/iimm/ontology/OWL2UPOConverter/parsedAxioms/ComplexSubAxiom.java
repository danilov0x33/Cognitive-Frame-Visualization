...
/**
 * Сложная субаксиомы - коньюнкция выражений или выражение, не являющееся
 * имемнованным классам.
 * 
 * @author Lomov P. A.
 *
 */
public class ComplexSubAxiom extends SubAxiom
{
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ComplexSubAxiom.class);
	...
	/**
	 * Список объектных свойств, присущих именованных классам субаксиомы, в виде
	 * пар <IRI свойства -- IRI значения>
	 */
	private ArrayList<PairOfIRI> objPrpList;

	/**
	 * Список типизированных свойств, присущих именованных классам субаксиомы, в
	 * виде пар <IRI свойства -- IRI типа свойства>
	 */
	private ArrayList<PairOfIRI> dtpPrpList;

	/**
	 * Карта пар: свойство --> именованный класс (входящий в субаксиому и потому
	 * привязываемый к ней).
	 */
	private ArrayList<ArrayList<IRI>> bindedClassList;

	/**
	 * Список объектных свойств, приведенных в самой аксиоме, в виде пар
	 * <IRI-свойства -- субаксиома>
	 */
	...
	/**
	 * Добавляет пару <свойство - значение> в список свойств классов, входящих в
	 * субаксиому.
	 * 
	 * @param prpIRI
	 * @param valIRI
	 */
	public void addToPrpList(IRI prpIRI, IRI valIRI,
	...
	/**
	 * Добавляет пару <свойство - субаксиома> в список свойств в субаксиоме.
	 * 
	 * @param prpIRI
	 * @param valIRI
	 * @deprecated
	 ...
	 * 
	 * @param prpExp
	 *            ограничение на свойство, которое определяет неименованный
	 *            класс
	 * @return
	 * @deprecated
	 * 
	 */
	public static OWLClassExpression getObjectPrpopertyRestrictionRANGE(
			OWLClassExpression prpExp)
	{
		/*
		 * Определяем тип ограничения на свойство, от этого зависит способ
		 * обработки. Приводим ограничение к узнанному типу, берем его
		 * внутреннюю часть и приводим ее к classExp
		 */
		...
		// if ( ((OWLRestriction)
		// prpExp).getClassExpressionType().equals(ClassExpressionType.DATA_HAS_VALUE)
		// )
		if (prpExp.getClassExpressionType().equals(
		...
		{
			rangeExp = (OWLClassExpression) ((OWLQuantifiedRestriction) prpExp)
					.getFiller();
		}
	...
	/**
	 * Возвращает IRI типа данных, которое связано с этим свойством. Т.е. само
	 * ограничение игнорируется.
	 * 
	 * @param prpExp
	 *            ограничение на свойство, которое определяет неименованный
	 *            класс
	 ...
	 * @deprecated
	 */
	public OWLDatatype getDataPrpopertyRestrictionRANGE(
			OWLClassExpression prpExp)
	{
		OWLDataProperty prp = (OWLDataProperty) ((OWLRestriction) prpExp)
				.getProperty();

		// для каждого свойства находим его RANGE - берем оттуда типы
		OWLDatatype datatype = OWLont.getOWLont().getDatatypeFromRange(
				prp.getIRI().toString());
	...
	/**
	 * Создаем фрагмент запроса из списка именованных классов аксиомы и
	 * переданной sparql переменной, привоенной субаксиоме.
	 * 
	 */
	public String getSparqlQueryPart(ArrayList<OWLClass> clsList, String SxAxVar)
	{
		...
		for (OWLClass cls : clsList)
		{
			...
			IRI conceptIRI = UPOont
					.getUPOont()
					.getConceptByIRIinAnnotationValue(
							IRI.create(ConstantsOntConverter.SKOS_HIDDEN_LABEL),
							cls.getIRI());
			...
			fragmentOfQuery = fragmentOfQuery
					+ (SxAxVar + " " + ConstantsOntConverter.IRI_RDF_TYPE_SHORT
							+ " <" + conceptIRI + ">" + ".\n");
	...
	/**
	 * @TODO название сложной субаксиомы зависит от названия субаксиомы из
	 *       другой части + от отношения с ней, пока учитывается только 1-е,
	 *       может 2-е и не нужно
	 */
	...
	protected String generateTitle()
	{
		...
		{
			namedTitle += cls.getIRI().getFragment()
					+ ConstantsOntConverter.UPO_TITLE_DELIMITER;
		}

		/* Формируем карту: свойство --- массив субаксиом [] */
		HashMap<IRI, ArrayList<SubAxiom>> map = this.getPropSubaxMap(this
				.getPrpFromAxList());
		...
		for (IRI propIRI : map.keySet())
		{
			for (SubAxiom subax : map.get(propIRI))
				...
				namedTitle += propIRI.getFragment()
		...
		namedTitle = namedTitle
				+ this.getDatatypePropNameFragment(this.getDtpPrpList());

		/* Отрезаем лишний делимитер (-) из конца аксиомы */
		namedTitle = namedTitle.length() > 0 ? namedTitle
				.substring(0, namedTitle
						.lastIndexOf(ConstantsOntConverter.UPO_TITLE_DELIMITER))
				: namedTitle;

		/* Обрамляем скобками субаксиому */
		// namedTitle = namedTitle.length()>0 ? "[" + namedTitle + "]" : null;
		namedTitle = namedTitle.length() > 0 ? UPOont
				.getSBrackedString(namedTitle) : null;

		return namedTitle;
	}

	/**
	 * Формирует карту: свойство --- массив субаксиом [] для полседующего
	 * формирования имени субаксиомы.
	 * 
	 * @return
	 */
	private HashMap<IRI, ArrayList<SubAxiom>> getPropSubaxMap(
			ArrayList<PrpIRIandSubAxiom> prpAndSubaxList)
	{
		HashMap<IRI, ArrayList<SubAxiom>> map = new HashMap<IRI, ArrayList<SubAxiom>>();
		ArrayList<SubAxiom> sbaxList;

		for (PrpIRIandSubAxiom pair : prpAndSubaxList)
		{
			if (map.containsKey(pair.propIRI)
					&& map.get(pair.getPropIRI()) != null)
			{
	...
	/**
	 * Добавляет субаксиому в ОПП и связывает ее концепт с концентапи из нее
	 * (другими субаксиомами, содержащимися в ней классами). Метод добавляет
	 * субаксиому, как ComplexSubAxiom и должен использоваться если нет более
	 * специфических способов добавления субаксиомы.
	 * 
	 * @param sbAx
	 * @return
	 */
	...
	public OWLNamedIndividual addSubaxiom(UserPresenOnt upo)
	{
		...
		OWLNamedIndividual sbAxIndividual = this
				.addConceptOfComplexSubAxiomToUPO(upo);
			...
			OWLNamedIndividual mainSubaxInd, IRI relationIRI, UserPresenOnt upo)
	{
		 ...
		 * SKOS:Related)
		 */
		upo.makeObjPrpOBetweenElements(mainSubaxInd.getIRI(), relationIRI,
				sbaxInd.getIRI(), false);
		return sbaxInd;
	}
	 ...
	 * @return
	 */
	private OWLNamedIndividual addConceptOfComplexSubAxiomToUPO(
			UserPresenOnt upo)
	{

		/* Создаем концепт-субаксиому в ОПП */
		OWLNamedIndividual newConcept = upo.addConcept(upo
				.getNewComplexSbAxiomIRI(this).toString());
		...
		OWLLiteral fragmentOfQuerylLiteral = upo.df.getOWLLiteral(
				fragmentOfQuery, "en");
		...
		upo.addAnnotation(ConstantsOntConverter.RDF_LABEL, labelLiteral,
				(OWLEntity) newConcept);
		// upo.addAnnotation(SKOS_Hidden_label, hiddenLabelLiteral, (OWLEntity)
		// newConcept);
		upo.addAnnotation(ConstantsOntConverter.UPO_SPARQL_VARIABLE_LABEL,
				varlLiteral, (OWLEntity) newConcept);
		upo.addAnnotation(
			...
			OWLNamedIndividual sbAxInd, UserPresenOnt upo)
	{
		...
		for (Iterator<PairOfIRI> it = this.getIteratorOfDataPropertyList(); it
				.hasNext();)
		{
			PairOfIRI pair = (PairOfIRI) it.next();
			LOGGER.info("   DtpPrp:" + pair.getFirst() + " Val:"
					+ pair.getFirst());
			/*
		...
		for (ArrayList<IRI> prpAndCls : this.getBindedClassList())
		{
			...
			IRI clsIndIRI = upo
					.getConceptOfSubAxiomFromUPObyIRIinHiddenLabel(prpAndCls
							.get(1));
			IRI prpIRI = prpAndCls.get(0);
			upo.makeObjPrpOBetweenElements(sbAxInd.getIRI(), prpIRI, clsIndIRI,
					false);
		}

		/* Привязываем субаксиомы */
		LOGGER.info("   =Binding sbAx with other Subaxiom:");
		for (Iterator<PrpIRIandSubAxiom> it = this.getIteratorOfprpFromAxList(); it
				.hasNext();)
		{
			PrpIRIandSubAxiom pair = (PrpIRIandSubAxiom) it.next();
			LOGGER.info("   PRP:" + pair.propIRI + " SbAx:"
					+ pair.subax.getTitle());
			OWLNamedIndividual newSbAx = pair.subax.addRelatedSubAxiom(sbAxInd,
					pair.propIRI, upo);
			/*-
	...
	/**
	 * Возвращает фрагмент названия субаксиомы из ее типизированных свойств и из
	 * диапазонов.
	 * 
	 * @param propDomainPairList
	 * @return
	 */
	private String getDatatypePropNameFragment(
			ArrayList<PairOfIRI> propDomainPairList)
	{
		String fragment = "";
		for (PairOfIRI pairOfIRI : propDomainPairList)
		{
			...
			fragment = fragment
					...
					+ UPOont.getFBrackedString(pairOfIRI.getFirst()
							.getFragment())
					+ ConstantsOntConverter.UPO_TITLE_DELIMITER;
		}
		return fragment;
