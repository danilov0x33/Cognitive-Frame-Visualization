...
/**
 * Отвечает за создание экземпляров классов в данном пакедже.
...
public class DataFactory
{
	 ...
	 * @return
	 */
	public SplitAxiom getSplitAxiom(OWLLogicalAxiom ax, Ontology ontOWL, UserPresenOnt ontUPO)
	{
	...
	static public boolean isCompound(OWLAxiom ax)
	{
		...
		if (!ax.isLogicalAxiom()) return false;
	...
	/**
	 * Возвращает конвертер OWL онтологии в онтологию пользовательского представления.
	 * @param ont
	 * @param ontUPO
	 * @return
	 */
	public OWL2UPOConverter getOWL2UPOConverter(Ontology ont, UserPresenOnt ontUPO)
	{
		OWL2UPOConverter conv = new OWL2UPOConverter(ont, ontUPO);
		return conv;
	}
	 ...
	 */
	/*
	public DatatypeSimpleSubAxiom getDatatypeSimpleSubAxiom(OWLDatatype dt, Ontology ont, UserPresenOnt ontUPO)
	{
		return new DatatypeSimpleSubAxiom(dt, ont, ontUPO);
	}
	*/
	
	AbstractAxiom getCloneAxiom(AbstractAxiom ax)
	{		
	 ...
	 * @return
	 */
	SimpleObjectRestrictionSubAxiom getSimpleObjectRestrictionSubAxiom(OWLQuantifiedRestriction restrExp)
	{	
	...
	/**
	 * Делает субаксиому из выражения, не включающего
	 * объединения других выражений.
	 * @param clExp
	 * @param ontOWL
	 * @param ontUPO
	 * @return
	 */
	ComplexSubAxiom getComplexSubaxiom(OWLClassExpression clExp)
	{	    
	    ...
	    sb.setBindedClassList(getBindedClassesMap(sb.getClsList(), clExp, OWLont.getOWLont()));
		
	    // Выбираем из онтологии свойства именованных классов для привязки к аксиоме
	    sb.setDtpPrpList(getDatatypePropertiesOfNamedClass(clExp, OWLont.getOWLont()));
	    sb.setObjPrpList(getObjectPropertiesOfNamedClass(clExp, OWLont.getOWLont()));
	    ...
	    /* TODO визитор возвращает также список ТИПИЗИРОВАННых свойств из субаксиомы
	     * но вроде этот список нигде не используется, хотя эти свойства нужно (возможно) также привязать к аксиоме */
	    ...
	    sb.setObjPrpList(removeDublicatesOfPropertiesFromList(sb.getObjPrpList(), sb.getPrpFromAxList()));
	    sb.setDtpPrpList(removeDublicatesOfPropertiesFromList(sb.getDtpPrpList(), sb.getPrpFromAxList()));
	    ...
	    /*TODO левой части тут никакой не будет - надо гдето в 
	     * другом месте генерить кусти запросов - может в Аксиоме
	...
	/**
	 * Создает субаксиомы из простых типизированных ограничений:
	 * DataSome datatype
	 * DataAll datatype
	 * DataMin/Max/Exact-cardinality datatype
	 * @param restrExp
	 * @return
	 */
	public SimpleDataRestrictionSubaxiom 
	getSimpleDataRestrictionSubaxiom(OWLQuantifiedRestriction restrExp)
	{
	 ...
	 * @return
	 */
	ParsedAxiom getParsedAxiom(SplitAxiom ax)
	{
	    ...
	    LOGGER.info("== END - parse left side: Subax quantity:" + parserAx.getlSideOfAx().size());
	    
	    LOGGER.info("== START - parse right side: ["+ax.rightOWLclassExpression+"]");
	    ax.getRightOWLclassExpression().accept(rSideParserVisitor);
	    parserAx.setrSideOfAx(rSideParserVisitor.getSbList());
	    LOGGER.info("== END - parse right side: Subax quantity:" + parserAx.getrSideOfAx().size());
	    
	    /* Если визитор не смог распарсить части из-за неучитывания некоторых 
	     * classExpression и венул пустые списки субаксиом, то не создаем субаксиому */
	    if (parserAx.getrSideOfAx().isEmpty() || parserAx.getlSideOfAx().isEmpty())
	    {
		LOGGER.info("!!! WARN !!! Axiom {} are not parsed", ax.originalAxiom);
		return null;
	    }
	    
	    /* Анализируем распарсенную аксиому и 
	     * Вызываем (если требуется) создание более специфической аксиомы*/
/*	    if (this.isSimpleRestrictionAxiom(parserAx))
	    		return getSimpleRestrictionAxiom(parserAx);
	 */   
	...
	/**
	 * Опредляет тип SKOS-связи, которая будет образована между 
	 * концептом и концептами-субаксиомами, определяющими его.
	 * @param axType
	 * @return
	 */
	private IRI getSKOSRelationType(ParsedAxiom ax)
	{
		...
		/* TODO Может будут еще какие-нибудь способы определения
		 * типа связи в скос. Пока возвращает только related. 
		 */
...
//		LOGGER.info("   ===getNamedClassFromExp==");
	...
	/**
	 * Получаем список пар: свойство --> именованных класс. 
	 * Для привязки последних к субаксиоме.
	 * @param clsList именованные классы из субаксиомы
	 * @return массив массивов с записями вида <IRI свойства --> IRI класса>
	 */
	private ArrayList<ArrayList<IRI>> getBindedClassesMap(ArrayList<OWLClass> clsList,
	 ...
	 * @return
	 */
	private OWLObjectProperty defineClassRelationType(OWLClass cls, OWLClassExpression exp, 
		Ontology ontOWL)
	{
		Boolean isFollow;
		OWLObjectProperty definedPrp=null;
		OWLObjectProperty broader = UPOont.getUPOont().df.getOWLObjectProperty(IRI.create(ConstantsOntConverter.SKOS_BROADER));
		OWLObjectProperty related = UPOont.getUPOont().df.getOWLObjectProperty(IRI.create(ConstantsOntConverter.SKOS_RELATED));

		/* ..для проверки вхождения - делаем аксиомы о вложенности и проверяем какая из них выводится
		 * взависимости от этого подбираем отношение */
		OWLSubClassOfAxiom classIsBroaderThanSubaxiom =	ontOWL.df.getOWLSubClassOfAxiom(exp, cls);
		isFollow = ontOWL.reas.isEntailed(classIsBroaderThanSubaxiom);
		if (isFollow)
		{
			definedPrp=UPOont.getUPOont().df.getOWLObjectProperty(IRI.create(ConstantsOntConverter.SKOS_BROADER));
		}
			
		OWLSubClassOfAxiom subaxiomIsBroaderThanClass =	ontOWL.df.getOWLSubClassOfAxiom(cls, exp);
		isFollow = ontOWL.reas.isEntailed(classIsBroaderThanSubaxiom);
		if (isFollow)
		{
			definedPrp=UPOont.getUPOont().df.getOWLObjectProperty(IRI.create(ConstantsOntConverter.SKOS_NARROWER));
		}

		/* если отношение не определено оно будет - SKOS:related*/
		definedPrp = (definedPrp==null) ? UPOont.getUPOont().df.getOWLObjectProperty(IRI.create(ConstantsOntConverter.SKOS_RELATED)) : definedPrp;
	...
	/**
	 * Возвращает список свойств и значений, присущих именованным 
	 * классам в субаксиоме. 
	 * @param ontOWL онтология, в которой ищутся свойства - исходная онтология
	 * @param subAx субаксиома для анализа
	 * @return
	 */

	public ArrayList<PairOfIRI> getObjectPropertiesOfNamedClass(OWLClassExpression subAx, Ontology ontOWL)
		{
			...
			{//for----------------------------------------------------		
				if (!clasExp.isAnonymous())
				{
					...
					Set<OWLObjectProperty> objPrpSet = 
							ontOWL.getObjectProperiesOfClass(owlClass.getIRI().toString(),true);
					
					for (OWLObjectProperty prp : objPrpSet)
					{
						Set<OWLClass> clsRANGEset = 
								ontOWL.getClassFromRange(prp.getIRI().toString());
						// формируем пару <свойство -- значение> и добавляем ее в список.
						for (OWLClass classInRange : clsRANGEset)
						{
							PairOfIRI newPrp_Val = new PairOfIRI(prp.getIRI(), classInRange.getIRI());
	...
	/**
	 * Возвращает список типизированных свойств и их типов, 
	 * присущих именованным классам в субаксиоме. 
	 * @param ontOWL онтология, в которой ищутся свойства - исходная онтология
	 * @param subAx субаксиома для анализа
	 * @return
	 * TODO надо доделать чтобы еще типизированные свойства брались
	 */

	public ArrayList<PairOfIRI> getDatatypePropertiesOfNamedClass(OWLClassExpression subAx, Ontology ontOWL)
		{
			ArrayList<PairOfIRI> prpList = new ArrayList<PairOfIRI>();
			Set<OWLClassExpression> axСNSet = subAx.asConjunctSet();
			...
			{		
				if (!clasExp.isAnonymous())
				{
					...
					Set<OWLDataProperty> dtpPrpSet = 
							ontOWL.getDatatypesProperiesOfClass(owlClass.getIRI().toString(),true);
					...
					for (OWLDataProperty prp : dtpPrpSet)
					{
						...
						{
							PairOfIRI newPrp_Val = new PairOfIRI(prp.getIRI(), datatype.getIRI());
							prpList.add(newPrp_Val);
	...
	/**
	 * Удаляет из списков свойств (от именованных классов) субаксиомы, 
	 * те что уже есть в самой аксиоме.
	 * @param dtpPrpList
	 * @return список без свойств, встретивщихся в субаксиоме.
	 */
	private ArrayList<PairOfIRI> removeDublicatesOfPropertiesFromList(ArrayList<PairOfIRI> prpList, 
		ArrayList<PrpIRIandSubAxiom> prpFromAxList)
	{
		...
		for (PrpIRIandSubAxiom prpFromSubAX : prpFromAxList)		
		{
			...
			{
				if (prpFromSubAX.propIRI.equals(prpFromList.fIRI))
				{
					shortPrpList.remove(prpFromList);
					LOGGER.info("   Rem: " +	prpFromList.fIRI);
				}			   
	 ...
	 * @param classIRI IRI класса, которому соответвует экземпляр в ОПП.
	 * @param upo
	 * @return
	 */
	public SimpleSubAxiom getSimpleSubAxiom(IRI classIRI, UserPresenOnt upo)
	{
	    ...
	    {
		LOGGER.error("!!! Error !!! Individual for class [] not found in UPO", classIRI);
		return null;		
	    }
	 ...
	 * @return
	 */
	public SimpleComplementSubAxiom	getSimpleComplementSubAxiom(IRI classIRI, UserPresenOnt upo)
	{
	    ...
	    /* Заполняем пустыми списками поля (субаксиома простая поэтому большинство списков
	     * пустые)*/
	 ...
	 * @return
	 */
	public ComplexComplementSubAxiom getComplexComplementSubAxiom(OWLClassExpression operandExp, 
		UserPresenOnt upo)
	{
	    ComplexComplementSubAxiom sbax = 
		    new ComplexComplementSubAxiom(this.getComplexSubaxiom(operandExp));
	    ...
	    sbax.setTitle("[" + ConstantsOntConverter.UPO_NEGATION_LABEL_TEMPLATE + tempTitle+"]");
